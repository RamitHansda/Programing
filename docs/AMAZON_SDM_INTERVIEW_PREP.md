...
**A:** Recognised early that I needed domain expertise I didn't have in-house, so I hired a Virtual CISO to guide the compliance framework — that was itself a deliberate resourcing decision to move faster without compromising quality. Together, we mapped every system against the ISO 27001 control framework — identified 40+ gaps across encryption at rest, access control policies, and incident response. I owned the engineering execution: designed DLP controls, defined the data classification policy, ran employee security training, and coordinated with external auditors. I drove the engineering changes directly: rotating KMS keys, enforcing mTLS between services, restricting DB-level write privileges on immutable ledger tables. Drove this end-to-end over 8 months while continuing to lead engineering delivery.
